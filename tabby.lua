	...
	while true do
		...
		elseif event[1]=="mouse_click" or (event[1]=="monitor_touch" and event[2]==state.side) then
			if event[4]==ty then
				if caseSize==2 then
					local getClicked=decision+math.ceil((event[3]-tx-1)/3)
					if getClicked==decision then if decision==1 then return value or previous else value=execSelection[decision](value,tonumber(event[2]),previous,wrapped,state,msg,line) end
					elseif getClicked>=1 and getClicked<=#msg then decision=getClicked 
					else return value or previous end
				else
					local getClicked=decision+event[3]-tx
					if getClicked==decision then if decision==1 then return value or previous else value=execSelection[decision](value,tonumber(event[2]),previous,wrapped,state,msg,line) end
					elseif getClicked>=1 and getClicked<=#msg then decision=getClicked 
...
local function execEvent(state,wrapped,event,eventOld)
	...
	if event[1]=="timer" then
		if event[2]==state.timerInstance and not state.colored then
			...
			state.timerInstance=os.startTimer(0.1)
...
1.  Use as a program, just run
    ...
    If you want to skip an argument "-" should work (so it will revert to default value) just write some kind of "not number" instead
...
use the internal "start" function to get the GUI
...
side
	...
	local function lookForMe(where,seen)
		...
		for k,v in pairs(where) do
			if type(v)=="table" then
				local check=true
				for k1,v1 in ipairs(seen) do
					if v1==v then check=false end
				end
				if check then
					if type(v.start)=="function" and string.dump(v.start)==string.dump(start) then return true end
					table.insert(seen,v)
					if lookForMe(v,seen) then return true end
				end
			end
		end
		return false
	...
	if args[1]~=nil or not ( block or lookForMe(_G)) then
		term.setCursorPos(1,1)
		term.clear()
		print([[
This message will not
be shown if Tabby is
loaded as API
(Will get printed 
when loading it!!!)
